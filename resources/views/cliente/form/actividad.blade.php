...
<div class="form-group col-xs-6">
	...
	{!!Form::select('resultado',['' => 'Selecciona el Resultado'] ,null,['class' => 'form-control','id'=>'resultados']) !!}
</div>

<div class="form-group" style="display:none;width: 40%" id="monto">
	...
	<input class="form-control" name="presupuesto" id="presupuesto"  type="text">
</div>
...
//Llenar combobox estado
	$("#tipoActividades").change(function(event){
		...
		$.get("estados/"+event.target.value+"",function(response,state){
			...
			{
				$("#estados").append("<option id='optionEstado' value='"+response[i].id+"'>"+response[i].nombre+"</option>");
			}
